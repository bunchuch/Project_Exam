        ...
        renderClude.length !== 0 ? (
            ...
            px-2 py-2 text-center md:text-base
             text-[12px]  bg-white rounded">{
              renderClude ?  renderClude.map((i,key)=><ul className="flex justify-center  items-center 
                underline text-purple-900 rounded-lg" key={key-1}>
                <div className="flex md:px-2   rounded-sm  ">
                <li className="border-1 px-2 py-3 cursor-pointer  divide-x-2 ">{key+1}. {i}</li>
                </div>
