...
export  const Render = () =>{
  ...
  return <div className="max-w-5xl mx-auto rounded-xl border-[1px] mt-9 p-4 border-blue-200">
    {
      showScore ? <div className="flex flex-col justify-center items-center space-y-3">
       <h1 className="text-center font-roboto font-extrabold text-xl">Congratulations guy 🤩</h1>
   <div>
    {
      api.map((x, key)=><>
       <p className="bg-blue-100 text-blue-600 rounded-md w-14 px-2">{x.type}</p>
      <p>{key + 1}.{x.question}</p>
      {x.type === 'mcq' ? <div className="flex">
       ...
       <span className="bg-green-100 text-green-700 rounded-md mt-1 mx-2 px-2 py-1.5">{x.answer}</span> </p>
        </div>}
   ...
   <button className="bg-blue-200 px-3 py-1.5  rounded-md text-blue-700" onClick={()=> {
    setCurrentQuestion(0)
    setShowSocre(false)}}>
      Get Back
   </button>
      ...
      py-2">{currentQuestion +1} .{change}
       {api[currentQuestion].type === 'blank'  ?
        <FillBlanks values={api[currentQuestion]?.answer} evnet={handleChange}
    ...
    <div className="flex flex-col ">
      {
        ...
        {api[currentQuestion].clude.map((x, id)=> <>
           ...
           onChange={()=>handlecheckAns(x.id , api[currentQuestion].id)} type="checkbox"
      ...
      }
      {
        ...
        font-semibold">
        {
          api[currentQuestion].clude.map((v)=><li className="border-[1px] rounded-md px-2 py-1 border-blue-400">
            {v.title}
          </li>)
        }
        </ul>
    ...
    <button className="bg-blue-200 px-3 py-1.5  rounded-md text-blue-700" onClick={handlenext}>next</button>
